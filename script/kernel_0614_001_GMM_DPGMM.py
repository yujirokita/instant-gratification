...
dpgmm0 = GaussianMixture(n_components=MAX_COMPONENTS, covariance_type='full', max_iter=1000, random_state=RANDOM_SEED)
dpgmm1 = GaussianMixture(n_components=MAX_COMPONENTS, covariance_type='full', max_iter=1000, random_state=RANDOM_SEED)
for i in range(512):
    print('.', end='')
    train_i = train[magic_tr==i][:,infomative_cols[i]]
    ...
    for trn_idx, val_idx in kfold.split(train_i, target_i):
        ...
        val_train = train_i[val_idx,:]
        dpgmm0.fit(trn_train[trn_target==0])
        dpgmm1.fit(trn_train[trn_target==1])
        oof_i[val_idx] = dpgmm1.score_samples(val_train) - dpgmm0.score_samples(val_train)
        pred_i += (dpgmm1.score_samples(test_i) - dpgmm0.score_samples(test_i)) / kfold.n_splits
