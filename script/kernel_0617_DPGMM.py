...
class GMMBinaryClassifier(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X, y):
        ...
        for n in self.n_clusters_list:
            gmm0 = BayesianGaussianMixture(n_components=n, covariance_type='full', random_state=self.random_state).fit(X[y==0])
            gmm1 = BayesianGaussianMixture(n_components=n, covariance_type='full', random_state=self.random_state).fit(X[y==1])
