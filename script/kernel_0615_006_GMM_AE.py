...
for n in range(1, MAX_COMPONENTS+1):
    ...
    for i in range(MAX_MAGIC_NO):
        ...
        # scaling
        data_i = np.concatenate([train_i, test_i], axis=0)
        scaler = MinMaxScaler().fit(data_i)
        train_i = scaler.transform(train_i)
        test_i = scaler.transform(test_i)

        # AE
        input_dim = data_i.shape[1]
        inputs = Input(shape=(input_dim,))
        encoded = Dense(input_dim-1, activation='relu')(inputs)
        encoded = Dense(input_dim-2, activation='relu')(encoded)
        decoded = Dense(input_dim-1, activation='relu')(encoded)
        decoded = Dense(input_dim, activation='sigmoid')(decoded)
        autoencoder = Model(inputs, decoded)
        encoder = Model(inputs, encoded)
        autoencoder.compile(optimizer='adadelta', loss='binary_crossentropy')
        autoencoder.fit(data_i, data_i, epochs=1000, shuffle=True, verbose=0)
        train_i = encoder.predict(train_i)
        test_i = encoder.predict(test_i)
