...
class GMMBinaryClassifier(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X, y):
        ...
        for n in self.n_clusters_list:
            gmm0 = GaussianMixture(n_components=n, covariance_type='full', random_state=self.random_state, tol=1e-5, max_iter=1000).fit(X[y==0])
            gmm1 = GaussianMixture(n_components=n, covariance_type='full', random_state=self.random_state, tol=1e-5, max_iter=1000).fit(X[y==1])
...
def train_score(use_pseudo=False, use_synthetic=False, gen_samples=True, n_samples_per_component=10):
    ...
    for i in range(MAX_MAGIC_NO):
        ...
        target_i = target_for_train[magic_tr==i]
...
#################################### STEP1 ####################################
# loacl parameters
target_for_train = np.copy(target)
max_clusters = 4
...
train_score(use_pseudo=False, use_synthetic=False, gen_samples=True, n_samples_per_component=100)
...
# train, score, and sample
train_score(use_pseudo=True, use_synthetic=True, gen_samples=True, n_samples_per_component=200)
...
# train and score
train_score(use_pseudo=True, use_synthetic=True, gen_samples=False)
