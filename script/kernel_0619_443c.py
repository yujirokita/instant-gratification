...
prev_sub = pd.read_csv(PREV_SUBMISSION_PATH)
test_pub = prev_sub.merge(test, how='left', on='id').drop(columns=['id', 'target'])
magic_tp = test_pub.pop('wheezy-copper-turtle-magic').values
target_tp = np.where(prev_sub.target.values>0.5, 1, 0)
...
train = train.values; test = test.values; test_pub = test_pub.values
...
class BinaryGMM(BaseEstimator):
    ...
    def fit(self, X, y):
        ...
        for n in self.n_clusters_list:
            gmm0 = GaussianMixture(n_components=n, covariance_type='full', tol=1e-2, random_state=self.random_state).fit(X[y==0])
            gmm1 = GaussianMixture(n_components=n, covariance_type='full', tol=1e-2, random_state=self.random_state).fit(X[y==1])
...
train_score(train, target, magic_tr, test, magic_te,
    use_pseudo=True, pseudo_label=target_tp, pseudo_data=test_pub, pseudo_magic=magic_tp,
    use_synthetic=False,
...
train_score(train, target_for_train, magic_tr, test, magic_te,
    ...
    gen_samples=True, n_samples_per_component=400)
