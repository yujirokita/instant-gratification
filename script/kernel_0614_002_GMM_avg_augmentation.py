...
gmm0 = GaussianMixture(n_components=1, covariance_type='full', random_state=RANDOM_SEED)
gmm1 = GaussianMixture(n_components=1, covariance_type='full', random_state=RANDOM_SEED)
for i in range(512):
    print('.', end='')
    train_i = train[magic_tr==i][:,infomative_cols[i]]
    ...
    for trn_idx, val_idx in kfold.split(train_i, target_i):
        ...
        trn_train0 = trn_train[trn_target==0]
        trn_train1 = trn_train[trn_target==1]
        trn_train0_ = np.array([trn_train0[np.random.randint(len(trn_train0), size=5),:].mean(axis=0) for i in range(n_samples_per_class)])
        trn_train1_ = np.array([trn_train1[np.random.randint(len(trn_train1), size=5),:].mean(axis=0) for i in range(n_samples_per_class)])
        
        gmm0.fit(trn_train0_)
        gmm1.fit(trn_train1_)
        oof_i[val_idx] = gmm1.score_samples(val_train) - gmm0.score_samples(val_train)
        pred_i += (gmm1.score_samples(test_i) - gmm0.score_samples(test_i)) / kfold.n_splits
    oof[magic_tr==i] = oof_i
    pred[magic_te==i] = pred_i
print()
print(f'SCORE (STEP-2): {roc_auc_score(target, oof_n)}')
