...
- stratified Kfold based on clustering (STEP1 only)
"""
import numpy as np
import pandas as pd
...
for n in range(1, MAX_COMPONENTS+1):
    ...
    for i in range(MAX_MAGIC_NO):
        ...
        # clustering
        label_i = np.zeros(len(train_i))
        clustering = GaussianMixture(n_components=n, covariance_type='full', random_state=RANDOM_SEED)
        label_i[target_i==0] = 2**clustering.fit_predict(train_i[target_i==0])
        label_i[target_i==1] = 3**clustering.fit_predict(train_i[target_i==1])

        for trn_idx, val_idx in kfold.split(train_i, label_i): # statify by cluster
...
for n in range(1, MAX_COMPONENTS+1):
    ...
    for i in range(MAX_MAGIC_NO):
        ...
        for trn_idx, val_idx in kfold.split(train_i, target_i): # statify by target
