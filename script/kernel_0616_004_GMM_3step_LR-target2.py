...
- STEP2: GMM with n_components=1..6
- STEP3: GMM with n_components=1..8
- y-flip
- pseudo labeling
...
pred = np.zeros(len(test))
for trn_idx, val_idx in kfold.split(oof_mat, target):
    comb = LogisticRegression(solver='lbfgs').fit(oof_mat[trn_idx,:], target2[trn_idx]) # modified target
    oof[val_idx] = comb.predict_proba(oof_mat[val_idx,:])[:,1]
    pred += comb.predict_proba(pred_mat)[:,1] / kfold.n_splits
