...
class GMMBinaryClassifier(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X, y):
        ...
        for n in self.n_clusters_list:
            gmm0 = GaussianMixture(n_components=n, covariance_type='full', random_state=self.random_state, tol=1e-1).fit(X[y==0])
            gmm1 = GaussianMixture(n_components=n, covariance_type='full', random_state=self.random_state, tol=1e-1).fit(X[y==1])
